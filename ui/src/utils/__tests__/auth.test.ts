import { setAuthToken, removeAuthToken, isTokenActive, getAccessToken, getCurrentUser } from '../auth';
...
describe('auth.ts - Utilidades de autenticación', () => {
    ...
    describe('getAccessToken', () => {
        it('debe retornar el access token cuando existe en localStorage', () => {
            const mockToken = {
            ...
            localStorageMock[tokenKey] = JSON.stringify(mockToken);
        ...
        it('debe retornar cadena vacía cuando no hay token en localStorage', () => {
            // localStorage vacío
            expect(localStorage.getItem(tokenKey)).toBeNull();
        ...
        it('debe retornar cadena vacía cuando el token es null', () => {
            localStorageMock[tokenKey] = 'null';
    ...
    describe('getCurrentUser', () => {
        ...
        beforeEach(() => {
            // Mockear Date.now() para tener control sobre la validez del token
            jest.spyOn(Date, 'now').mockReturnValue(FIXED_TIME);
        });
        ...
        it('debe retornar un usuario cuando hay un token válido', () => {
            const validToken = {
        ...
        it('debe retornar undefined y eliminar el token cuando ha expirado', () => {
            const expiredToken = {
                accessToken: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test.token',
        ...
        it('debe retornar un usuario con los datos correctos del payload del JWT', () => {
            const validToken = {
                accessToken: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test.token',
                notBeforeTimestampInMillis: FIXED_TIME - 1000,
                expirationTimestampInMillis: FIXED_TIME + 1000000,
            };
